...
export default class TablePaginator extends Component {
  ...
  renderPage() {
    ...
    items.sort((a, b) => {
      let dispA = cols[currCol].display(a),
        dispB = cols[currCol].display(b);
      (dispA.props ? dispA.props.children : dispA).toString().localeCompare(
        (dispB.props ? dispB.props.children : dispB).toString()
      ) * (ascDesc ? 1 : -1)
    });
