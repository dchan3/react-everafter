...
export default function PaginatorControls() {
  let { state, dispatch } = useContext(PaginatorControlContext),
    ...
    state, truncated = truncate ?
      truncatePageList(maxPages || pages.length,
        maxPageTabs, currentPage) : undefined;
  ...
  return <PaginatorContainer>
    <PaginatorList>
      {currentPage > 1 && <PaginatorButton onClick={() => ({ type: 'page',
        val: '-' })}>
        <PaginatorNumber>{'<'}</PaginatorNumber>
      </PaginatorButton> || null}
      {truncated ? (truncated.map((n, key) => <PaginatorButton key {...{
        activeTabColor }} onClick={(n !== null && !!n) ?
        () => dispatch({ type: 'page', val: n }) : null}
      className={(n !== null && !!n )? isActive(n) : ''}>
        <PaginatorNumber>{(n !== null && !!n) ? n : '⋯'}</PaginatorNumber>
      </PaginatorButton>)) : generateArray(pages.length).map(i => (
        <PaginatorButton key {...{ activeTabColor }}
